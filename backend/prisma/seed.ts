// ============================================================
// Xamle Civic — Prisma Seed Script
// Run: pnpm --filter @xamle/api db:seed
// ============================================================

import { PrismaClient, PolicyStatus, PolicyTheme, WorkflowStatus, UserRole, UserLevel, SourceType, SenegalRegion } from '@prisma/client';
  ...
  const policies = await Promise.all([
    ...
    prisma.policy.create({
      data: {
        ...
        theme: PolicyTheme.INFRASTRUCTURE,
        status: PolicyStatus.IN_PROGRESS,
        workflowStatus: WorkflowStatus.PUBLISHED,
        budget: 750000000000,
  ...
  for (const policy of policies) {
    await prisma.statusHistory.create({
      data: {
        policyId: policy.id,
        fromStatus: null,
        toStatus: PolicyStatus.NOT_STARTED,
        changedBy: superAdmin.id,
        reason: 'Création initiale de la politique publique',
      },
    });
    if (policy.status !== PolicyStatus.NOT_STARTED) {
      await prisma.statusHistory.create({
        data: {
          policyId: policy.id,
          fromStatus: PolicyStatus.NOT_STARTED,
  ...
  const contributions = await Promise.all([
    prisma.contribution.create({
      data: {
        ...
        type: 'TESTIMONY',
        content:
          'Dans ma commune de Gossas (Fatick), la CMU a bien été mise en place mais les agents de santé manquent de formation sur les procédures d\'enrôlement. De nombreux ménages éligibles ne sont pas encore couverts faute d\'information.',
        location: 'Gossas, Fatick',
        region: SenegalRegion.FATICK,
        status: 'APPROVED',
        reliability: 4,
    ...
    prisma.contribution.create({
      data: {
        ...
        type: 'TESTIMONY',
        content:
          'Les travaux de l\'hôpital régional de Kolda sont à l\'arrêt depuis 3 mois. Le site est abandonné et aucune information n\'est disponible sur la reprise des travaux.',
        location: 'Kolda',
        region: SenegalRegion.KOLDA,
        status: 'APPROVED',
        reliability: 5,
        moderatorId: moderator.id,
        reviewedAt: new Date(),
    ...
    prisma.contribution.create({
      data: {
        ...
        type: 'TESTIMONY',
        content:
          'Dans la région de Podor, les semences certifiées promis dans le cadre du PNAR n\'ont pas été distribuées cette saison. Les paysans ont dû utiliser leurs propres semences non sélectionnées.',
        location: 'Podor, Saint-Louis',
        region: SenegalRegion.SAINT_LOUIS,
        status: 'PENDING',
        reliability: 0,
      },
    }),
